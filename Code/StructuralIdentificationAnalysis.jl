...
#Shows global identifiability for M1.
...
#Shows global identifiability for M2
...
M5 = @ODEmodel(
    U'(t) = -beta*U(t)*V(t),
    I'(t) = beta*U(t)*V(t) - delta_I * T(t)*I(t),
    V'(t) = p*I(t) - c*V(t),
    T'(t) = s_T + r*T(t)*V(t)/(V(t)+k_T)-c_T*(1/(1+V(t)^2))-d_T*T(t),
    y1(t) = V(t),
    y2(t) = T(t)
...
M5_identifiability = assess_identifiability(M5)
...
MC1 = @ODEmodel(
    V'(t) = p*V(t)*(1-V(t)/k_V)*(1/(1+(K*T(t))^2)) - c_V*V(t)*T(t),
    T'(t) = s_T + r*T(t)*V(t)/(V(t)+k_T)-d_T*T(t),
    y1(t) = V(t),
    y2(t) = T(t)
)
...
#=
Results:
    M1: all globally identifiable
    M2: all globally identifiable
    M3: s_N and k_T locally, all others are globally identifiable
    M4: all globally identifiable.
    M5: p nonidentifiable, all others identifiable.
=#