...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 10 19:41:17 2023
...
def modelC2(y,t,p,k_V,c_V,s_T,r,c_T,d_T):
    V,T = y
    
    dVdt = p*V*(1-V/k_V)-c_V*V*T
    dTdt = s_T + r*T*V-d_T*(1/(1 + V**2))*T - c_T*T
...
def ModelC2_RMSLE(x,times,step_res,Viral_Data,CD8_Data,
    ...
    p,k_V,c_V,r,c_T,d_T,T_0,V_0 = x #This is the order the parameters should be input.
    ...
    predictions = sp.integrate.odeint(modelC2,
                                         y0,
                                         t,
                                         args=(p,k_V,c_V,s_T,r,c_T,d_T)
...
def ModelC2_AICc(x,times,step_res,Viral_Data,CD8_Data,
                 Viral_Data_Col='Viral Titer (Pfu/ml)',
...
def ModelC2_Predict(x,times,step_res):
    p,k_V,c_V,r,c_T,d_T,T_0,V_0 = x
    s_T = (c_T +d_T) * T_0
    y0 = [V_0,T_0]
...
def ModelC2_ViralClearanceTime(x,times,step_res,threshhold = 10.0):
    p,k_V,c_V,r,c_T,d_T,T_0,V_0 = x
    s_T = (c_T +d_T) * T_0
    y0 = [V_0,T_0]
...
def ModelC2_TotalViralLoad(x,times,step_res,threshhold = 10.0):
    p,k_V,c_V,r,c_T,d_T,T_0,V_0 = x
    s_T = (c_T +d_T) * T_0
    y0 = [V_0,T_0]
    ...
    dt = t[2]-t[1]
    #print(predictions)
    #print(max(predictions[valid_indices,0][0]))
    total = np.sum(predictions[valid_indices,0][0]*dt)
    return total

def ModelC2_ExcessCTL(x,times,step_res,threshhold = 10.0):
    p,k_V,c_V,r,c_T,d_T,T_0,V_0 = x
    s_T = (c_T +d_T) * T_0
    y0 = [V_0,T_0]
