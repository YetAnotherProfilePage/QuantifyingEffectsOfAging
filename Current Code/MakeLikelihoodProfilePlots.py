...
def MakeMainLikelihoodProfilePlots(model_name,group,plottitle,label_indices, params_labels, model_color = 'black', alpha=None, fits_folder = Fitted_Folder, save_folder = Destination_Folder):
    if alpha==None:
        ALPHA = 1.0
    else:
        ALPHA = alpha
    ...
    for i in range(num_params):
        ...
        axs[row,column].plot(temp_dfs[i][active_params[i]],temp_dfs[i]['RMSLE'],marker='o',color=model_color,alpha=ALPHA)
...
if __name__ == "__main__":
    ...
    MakeMainLikelihoodProfilePlots("ModelA3","Adult","Adult Likelihood Profiles (MA3)",ModelA3_Fitted_Params_Indices,ModelA3_Params_Labels,model_color="blue",alpha=0.5)
    MakeMainLikelihoodProfilePlots("ModelB1","Aged","Aged Likelihood Profiles (MB1)",ModelB1_Fitted_Params_Indices,ModelB1_Params_Labels,model_color="darkorange")
