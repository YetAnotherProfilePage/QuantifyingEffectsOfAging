...
def Make_Model_Group_Local_Sensitivity_Plot(model,group,filenames,color=None,alpha=None):
    if color == None:
    ...
    for i in range(0,num_params):
        for j in range(0, len(OUTCOMES)):
            axs[j,i].plot(dfs[i][params[i]],dfs[i][OUTCOMES[j]],color=COLOR,alpha=ALPHA)
...
if __name__ == "__main__":
    ...
    Make_Model_Group_Local_Sensitivity_Plot("MA3","adult",filenames,color="blue",alpha=0.5)
    Make_Model_Group_Local_Sensitivity_Plot("MB1","aged",filenames,color="darkorange")
