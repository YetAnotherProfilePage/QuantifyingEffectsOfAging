...
def modelD4(y,t,beta,d_I,p,c,s_T,d_T,r,c_T,K):
    U,I,V,T = y
    
    dUdt = -beta*U*V
    dIdt = beta*U*V-d_I*T*I
    dVdt = p*I*(1/(1+(K*T)**2))-c*V
    dTdt = s_T - d_T*T + r*T*V-c_T*(1/(1+V**2))*T
    
    dydt = [dUdt,dIdt,dVdt,dTdt]
    return dydt

def ModelD4_RMSLE(x,times,step_res,Viral_Data,CD8_Data,
    ...
    beta,d_I,p,c,d_T,r,c_T,K,U_0,I_0,V_0,T_0 = x #This is the order the parameters should be input.
    ...
    predictions = sp.integrate.odeint(modelD4,
                                         y0,
                                         t,
                                         args=(beta,d_I,p,c,s_T,d_T,r,c_T,K)
...
def ModelD4_AICc(x,times,step_res,Viral_Data,CD8_Data,
                 Viral_Data_Col='Viral Titer (Pfu/ml)',
...
def ModelD4_Predict(x,times,step_res):
    beta,d_I,p,c,d_T,r,c_T,K,U_0,I_0,V_0,T_0 = x #This is the order the parameters should be input.
    #We leave all parameters as inputs in order to do sensitivity analysis later.
...
def ModelD4_ViralClearanceTime(x,times,step_res,threshhold = 10.0):
    beta,d_I,p,c,d_T,r,c_T,K,U_0,I_0,V_0,T_0 = x #This is the order the parameters should be input.
    #We leave all parameters as inputs in order to do sensitivity analysis later.
...
def ModelD4_TotalViralLoad(x,times,step_res,threshhold = 10.0):
    beta,d_I,p,c,d_T,r,c_T,K,U_0,I_0,V_0,T_0 = x #This is the order the parameters should be input.
    #We leave all parameters as inputs in order to do sensitivity analysis later.
...
def ModelD4_ExcessCTL(x,times,step_res,threshhold = 10.0):
    beta,d_I,p,c,d_T,r,c_T,K,U_0,I_0,V_0,T_0 = x #This is the order the parameters should be input.
    #We leave all parameters as inputs in order to do sensitivity analysis later.
